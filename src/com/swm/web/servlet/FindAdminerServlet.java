...
@WebServlet("/FindAdminerServlet")
//查询书籍
public class FindAdminerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setCharacterEncoding("utf-8");
			...
			//接受参数
		    FindAdminerForm form=new FindAdminerForm();
		    try {
				BeanUtils.populate(form,request.getParameterMap());
				IAdminerService adminerService = new AdminerServiceImpl();
				List<Adminer> Adminer =adminerService.findUser(form);
				
				//3.返回list列表页面
				//重新获取 book数据
				
				
				request.setAttribute("Adminer", Adminer);
